...
NE_CORE_DECLARE_PACKAGE(ne_io, 0, 0);
...
NE_CORE_API void (*ne_io_get_input)(uint64_t *result, ne_core_stream *stream_out);

// Must only support: write, flush, get_position, get_length.
NE_CORE_API void (*ne_io_get_output)(uint64_t *result, ne_core_stream *stream_out);
