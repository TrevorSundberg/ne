...
//      the creation, etc.
NE_API void (*ne_filesystem_open_file)(uint64_t *result, const char *path,
                                       ne_filesystem_io io,
...
NE_API ne_filesystem_entry_type (*ne_filesystem_get_type)(uint64_t *result,
                                                          const char *path);

NE_API uint64_t (*ne_filesystem_get_last_write_time)(uint64_t *result,
                                                     const char *path);
NE_API void (*ne_filesystem_set_last_write_time)(uint64_t *result,
                                                 const char *path,
                                                 uint64_t time);

NE_API const char *(*ne_filesystem_read_symlink)(uint64_t *result,
                                                 const char *path);

// Returns the size of a file in bytes.
//    ne_filesystem_result_file_does_not_exist:
//      The path specified does not point at a valid file.
NE_API uint64_t (*ne_filesystem_file_size)(uint64_t *result, const char *path);
...
NE_API void (*ne_filesystem_delete)(uint64_t *result, const char *path);

// Moves a file from one path to another path. If the filename is different,
// this renames the file.
NE_API void (*ne_filesystem_move_rename)(uint64_t *result,
...
NE_API void (*ne_filesystem_enumerator)(uint64_t *result, const char *path,
...
NE_API const char *(*ne_filesystem_translate_ne_to_os)(uint64_t *result,
                                                       const char *path);
...
NE_API const char *(*ne_filesystem_translate_os_to_ne)(uint64_t *result,
                                                       const char *path);
...
// location).
NE_API const char *(*ne_filesystem_set_working_directory)(uint64_t *result,
                                                          const char *path);
...
// possible.
NE_API const char *(*ne_filesystem_get_public_user_writable_directory)(
    uint64_t *result);

// Get a directory that your application will have permission to write to.
// This is generally a hidden or local directory, specific to a user if
// possible.
NE_API const char *(*ne_filesystem_get_private_user_writable_directory)(
    uint64_t *result);
...
NE_API const char *(*ne_filesystem_dialog)(
