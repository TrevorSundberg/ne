...
// This way is chosen to keep code platform agnostic. To translate between the current
// operating system format use the translate functions.
...
// This function should only be used for parsing a file path provided by the user.
NE_API const char* (*ne_filesystem_translate_ne_to_os)(const char* path);

NE_API void (*ne_filesystem_open_file)(uint64_t* result, ne_filesystem_mode mode, const char* path, ne_core_stream* stream_out);
...
NE_API const char* (*ne_filesystem_set_working_directory)(uint64_t* result, const char* path);
